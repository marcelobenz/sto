            ...
            <div class="form-group">
                @foreach($permisos as $permiso)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="permisos[]" value="{{ $permiso->id_permiso }}" 
                            {{ in_array($permiso->id_permiso, $permisosAsignados) ? 'checked' : '' }}>
                        <label class="form-check-label" for="permisos[]">{{ $permiso->permiso }} - {{ $permiso->descripcion }}</label>
                    </div>
                @endforeach
            </div>
