@php
    $selectStyle =
        "width: 100%; border-radius: 0.375rem; padding-block: 1px; padding-inline: 2px; padding: 1px 0px;";
...
{{ $valorCampo }}</textarea
            >
            <label
                id="charCountInfo-{{ $c->id_campo }}"
                style="font-size: 14px; color: darkslategray"
            >
            ...
            @else
                @if ($c->limite_minimo && $c->limite_maximo)
                    ...
                    <label style="font-size: 14px; color: darkslategray">
                        Límite caracteres: Min {{ $c->limite_minimo }} / Max
                        {{ $c->limite_maximo }}
                    </label>
                @else
                    <input
