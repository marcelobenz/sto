<div>
    ...
    <div
        class="border-separate border border-slate-400 p-4"
        style="display: flex; flex-direction: column; gap: 15px"
    >
        <form
            id="form-actualizar-datos-campo"
            method="POST"
            action="{{ route("secciones-multinota.actualizarDatosCampo") }}"
        >
            @csrf
            <input
                type="hidden"
                name="input-titulo-seccion-backup"
                id="input-titulo-seccion-backup"
            />
            <div style="display: flex; justify-content: space-between">
                ...
                >
                    @foreach ($tipos as $t)
                        <option
                            value="{{ $t->tipo }}"
                            @selected($campoSelected->tipo == $t->tipo)
                        >
                            @if ($t->tipo == "STRING")
                                Texto
            ...
            <div>
                ...
                <x-text-input
                    ...
                    max="12"
                    name="tamaño"
                    :value="__($campoSelected->dimension)"
                    required
                />
                <label>(solo valores de 1 a 12)</label>
            </div>
            <label>
                Obligatorio
                <input
                    type="checkbox"
                    name="obligatorio"
                    {{ $campoSelected->obligatorio == 1 ? "checked" : "" }}
                />
            </label>
                ...
                <div>
                    <div>
                        ...
                        <input
                            ...
                            id="lleva-mascara"
                            name="lleva-mascara"
                            {{ $campoSelected->mascara != null ? "checked" : "" }}
                        />
                    </div>
                    <div
                        id="lleva-mascara-input-container"
                        style="
                            display: {{ $campoSelected->mascara != null ? "block" : "none" }};
                        "
                    >
                        <input
                            {{ $campoSelected->mascara != null ? "required" : "" }}
                            id="lleva-mascara-input"
                            type="text"
                            value="{{ old("mascara", $campoSelected->mascara) }}"
                            name="lleva-mascara-input-container"
                            placeholder="Máscara"
                        />
                    </div>
                </div>
            ...
            @if ($campoSelected->tipo == "STRING" || $campoSelected->tipo == "INTEGER" || $campoSelected->tipo == "TEXTAREA")
                <div>
                    <div>
                        <label for="limitar-caracteres">
                            Limitar caractéres
                        </label>
                        <input
                            type="checkbox"
                            id="limitar-caracteres"
                            name="limitar-caracteres"
                            {{ $campoSelected->limite_minimo != null && $campoSelected->limite_maximo != null ? "checked" : "" }}
                        />
                    </div>
                    <div
                        id="limitar-caracteres-input-container"
                        style="
                            display: {{ $campoSelected->limite_minimo != null && $campoSelected->limite_maximo != null ? "block" : "none" }};
                        "
                    >
                        <input
                            {{ $campoSelected->limite_minimo != null ? "required" : "" }}
                            min="1"
                            max="9999"
                            type="number"
                            value="{{ old("limite_minimo", $campoSelected->limite_minimo) }}"
                            id="limitar-caracteres-input-min"
                            name="limitar-caracteres-input-min"
                            placeholder="Mínimo"
                        />
                        <input
                            {{ $campoSelected->limite_maximo != null ? "required" : "" }}
                            min="1"
                            max="9999"
                            type="number"
                            value="{{ old("limite_maximo", $campoSelected->limite_maximo) }}"
                            id="limitar-caracteres-input-max"
                            name="limitar-caracteres-input-max"
                            placeholder="Máximo"
                        />
                    </div>
                </div>
...
@push("scripts")
    <script>
        ...
        $(document).ready(function () {
            if (
                document.getElementById('select-tipos').value == 'LISTA' ||
                document.getElementById('select-tipos').value ==
                    'CAJAS_SELECCION'
            ) {
            ...
            $('#select-tipos').on('change', function () {
                ...
                fetch(
                    `/secciones-multinota/getOpcionesFormTipoCampo/${selectedValue}`,
                )
                    .then((response) => response.text())
                    .then((data) => {
            ...
            $('#opciones-campo-container').on(
                ...
                function () {
                    ...
                    fetch(
                        `/secciones-multinota/deleteOpcionCampo/${idOpcionCampo}`,
                    )
                        .then((response) => response.text())
                        .then((data) => {
                            document.getElementById(
        ...
        document
            .getElementById('form-actualizar-datos-campo')
            .addEventListener('submit', function () {
                const inputTituloSeccion = document.getElementById(
                    'input-titulo-seccion',
                ).value;
                console.log('aver: ', inputTituloSeccion);
                document.getElementById('input-titulo-seccion-backup').value =
                    inputTituloSeccion;
            });
        ...
        if (campoSelected.tipo === 'STRING') {
            document
                .getElementById('lleva-mascara')
                .addEventListener('click', function () {
                    let inputContainer = document.getElementById(
                        'lleva-mascara-input-container',
                    );
        ...
        if (
            ...
            campoSelected.tipo === 'INTEGER' ||
            campoSelected.tipo === 'TEXTAREA'
        ) {
            document
                .getElementById('limitar-caracteres')
                .addEventListener('click', function () {
                    let inputContainer = document.getElementById(
                        'limitar-caracteres-input-container',
                    );
                    let inputMinimo = document.getElementById(
                        'limitar-caracteres-input-min',
                    );
                    let inputMaximo = document.getElementById(
                        'limitar-caracteres-input-max',
                    );
