...
@section("content")
    ...
    >
        ...
        <div
            id="ruta-paso-tramite"
            style="border: 2px solid gray; margin-bottom: 0.5rem"
        >
...
@push("scripts")
    <script>
        ...
        function inicializarInputsCajaSeleccionMultiple() {
            document
                .querySelectorAll('.choices-multiselect')
        ...
        function actualizarCharCount(textarea) {
            if (textarea) {
                ...
                const charCountInfoElement = document.getElementById(
                    `charCountInfo-${textarea.id}`,
                );
        ...
        function mostrarCustomTooltip(targetElement, message) {
            ...
            ['click', 'keydown', 'change'].forEach((event) =>
                targetElement.addEventListener(event, eliminarCustomTooltip, {
                    once: true,
                }),
            );
        }
        ...
        function refreshBotonesTrasCheckboxDeclaracion(checked) {
            fetch(
                '{{ route("instanciaTramite.handleCheckConfirmarDeclaracion") }}',
                {
                    method: 'POST',
                    headers: {
        ...
        $(document).ready(function () {
            ...
            async function validarDatosSeccionInicio() {
                ...
                if (!isValid) {
                    throw new Error(
                        'Debe completar todos los datos obligatorios correctamente',
                    );
                }
            }
            ...
            function validarDatosSeccionSolicitante() {
                ...
                for (const input of inputs) {
                    ...
                    if (input.id === 'correoSolicitanteRepetido') {
                        let correo =
                            document.getElementById('correoSolicitante').value;
                        let correoRepetido = document.getElementById(
                            'correoSolicitanteRepetido',
                        );

                        // Evitar problemas de case
                        correo = correo.trim().toLowerCase();
                        correoRepetido.value = correoRepetido.value
                            .trim()
                            .toLowerCase();
                        ...
                        if (correo !== correoRepetido.value) {
                            ...
                            correoRepetido.addEventListener(
                                'input',
                                () => {
                                    correoRepetido.setCustomValidity('');
                                },
            ...
            function guardarDatosSeccionSolicitante() {
                ...
                const tipoCaracter = document.getElementById(
                    'tipoCaracterSolicitante',
                ).value;
                const correo =
                    document.getElementById('correoSolicitante').value;
                const nombreCalle =
                    document.getElementById('nombreCalleLoc').value;
                const numeroCalle =
                    document.getElementById('numeroCalleLoc').value;
                const provincia = document.getElementById('provinciaLoc').value;
                const localidad = document.getElementById('localidadLoc').value;
                const codigoPostal =
                    document.getElementById('codigoPostalLoc').value;
                ...
                fetch(
                    '{{ route("instanciaTramite.guardarDatosSeccionSolicitante") }}',
                    {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    ...
                    if (!res.ok) {
                        console.error(
                            'Error al guardar datos del representante',
                        );
                    }
                });
            }
            ...
            function validarDatosSeccionDatosACompletar() {
                inputsSecciones = campos.map((c) =>
                    document.getElementById(`${c.id_campo}`),
                );
                ...
                for (const input of inputsSecciones) {
                    ...
                    if (input.classList.contains('choices-multiselect'))
                        continue;
                ...
                if (isValid) {
                    ...
                    for (const select of selects) {
                        ...
                        if (selectedOptions.length === 0) {
                            ...
                            const choicesInner = select
                                .closest('.choices')
                                ?.querySelector('.choices__inner');
                            if (choicesInner) {
                                mostrarCustomTooltip(
                                    choicesInner,
                                    'Debe seleccionar al menos una opción',
                                );
                                choicesInner.focus();
            ...
            function guardarDatosSeccionDatosACompletar() {
                ...
                for (const input of inputsSecciones) {
                    ...
                    if (input.multiple) {
                        ...
                        valor = Array.from(input.selectedOptions).map(
                            (opt) => opt.value,
                        );
                    } else {
                        // Campos comunes
                ...
                fetch(
                    '{{ route("instanciaTramite.guardarDatosSeccionDatosACompletar") }}',
                    {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    ...
                    if (!res.ok) {
                        console.error(
                            'Error al guardar datos de las secciones asociadas',
                        );
                    }
                });
            }
            ...
            function guardarDatosSeccionInformacionAdicional() {
                ...
                fetch(
                    '{{ route("instanciaTramite.guardarDatosSeccionInformacionAdicional") }}',
                    {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                ...
                try {
                    ...
                    switch (ordenActual) {
                        ...
                        case 4:
                            if (persona === 'Juridica') {
                                ...
                                if (!result) {
                                    const error = new Error(
                                        'Debe adjuntar la documentación solicitada',
                                    );
                                    error.mostrar = true;
                    ...
                    fetch('/instanciaTramite/avanzarPaso')
                        ...
                        .then((data) => {
                            document.getElementById(
                                'pasos-container',
                            ).innerHTML = data.htmlPasos;
                            document.getElementById(
                                'ruta-paso-tramite',
                            ).innerHTML = data.htmlRuta;
                            document.getElementById(
                                'botones-avance-tramite',
                            ).innerHTML = data.htmlBotones;
                            ...
                            actualizarCharCount(
                                document.getElementById('informacionAdicional'),
                            );
                            for (const input of inputsSecciones) {
                ...
                async function (e) {
                    ...
                    try {
                        const response = await fetch(
                            '/instanciaTramite/registrarTramite',
                        );
                ...
                fetch('/instanciaTramite/retrocederPaso')
                    ...
                    .then((data) => {
                        document.getElementById('pasos-container').innerHTML =
                            data.htmlPasos;
                        document.getElementById('ruta-paso-tramite').innerHTML =
                            data.htmlRuta;
                        document.getElementById(
                            'botones-avance-tramite',
                        ).innerHTML = data.htmlBotones;
