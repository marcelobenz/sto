...
@section("content")
    ...
    <div class="container-fluid">
        <div class="row gx-4 align-items-start mt-4">
            <div
                class="d-flex justify-content-between align-items-center w-100 flex-wrap bg-light p-3 rounded"
            >
                <div class="d-flex flex-grow-1 gap-2">
                    <div class="border rounded p-2 flex-grow-1 bg-white">
                        Trámite Nro
                        <strong>{{ $idTramite }}</strong>
                    ...
                    <div class="border rounded p-2 flex-grow-1 bg-white">
                        Estado actual:
                        <span
                            class="badge @if ($tramiteInfo->estado_actual === "Aprobado")
                                bg-success
                            @elseif ($tramiteInfo->estado_actual === "Rechazado")
                                bg-danger
                            @elseif ($tramiteInfo->estado_actual === "Dado de Baja")
                                bg-secondary
                            @elseif ($tramiteInfo->estado_actual === "Iniciado")
                                bg-primary
                            @elseif ($tramiteInfo->estado_actual === "Finalizado")
                                bg-success
                            @else
                                bg-dark
                            @endif"
                    ...
                    <div class="border rounded p-2 flex-grow-1 bg-white">
                        Prioridad:
                        <span
                            class="badge @if (strtolower($tramiteInfo->prioridad) === "baja")
                                bg-success
                            @elseif (strtolower($tramiteInfo->prioridad) === "normal")
                                bg-warning
                                text-dark
                            @elseif (strtolower($tramiteInfo->prioridad) === "alta")
                                bg-danger
                            @else
                                bg-secondary
                            @endif"
                    ...
                    <div class="border rounded p-2 flex-grow-1 bg-white">
                        Asignado a:
                        <strong>
                            {{ $tramiteInfo->nombre_usuario }}
                            {{ $tramiteInfo->apellido_usuario }}
                        </strong>
                    </div>
                </div>
                <div class="ms-3 d-flex gap-1">
                    <button class="btn btn-warning" title="Reasignar">
                        <i class="fas fa-random"></i>
                    </button>
                    <button
                        class="btn btn-warning"
                        data-bs-toggle="modal"
                        data-bs-target="#modalCambiarPrioridad"
                ...
                <div class="container-fluid px-3">
                    ...
                    <table class="table table-striped table-hover">
                        <thead class="table-dark">
                            <tr>
                                <th class="text-center fs-4" colspan="4">
                                    Información
                                </th>
                            </tr>
                        </thead>
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                <tr>
                                    <td>
                                        <form
                                            action="{{ route("comentario.guardar") }}"
                                            method="POST"
                                        >
                                            @csrf
                                            <input
                                            ...
                                            <input
                                                type="hidden"
                                                name="id_usuario_administador"
                                                value="{{ auth()->user()->id ?? "152" }}"
                                            />
                                            <div class="mb-3">
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @foreach ($tramiteArchivo as $tramiteArchivo)
                                    <tr>
                                        <td>
                                            {{ $tramiteArchivo->fecha_alta }}
                                        </td>
                                        <td>{{ $tramiteArchivo->nombre }}</td>
                                        <td>
                                            {{ $tramiteArchivo->descripcion }}
                                        </td>
                                        <td class="text-center">
                                            <a
                                                href="{{ route("archivo.descargar", $tramiteArchivo->id_archivo) }}"
                                                title="Descargar"
                                            >
                                                <i
                                ...
                                <tbody>
                                    @forelse ($historialTramite as $evento)
                                        <tr>
                                            ...
                                            <td>
                                                {{ date("d/m/Y H:i", strtotime($evento->fecha_alta)) }}
                                            </td>
                                            <td class="text-center">
                                                {{ $evento->legajo }} -
                                                {{ $evento->usuario }}
    ...
    <div
        class="modal fade"
        id="modalCambiarPrioridad"
        tabindex="-1"
        aria-labelledby="modalCambiarPrioridadLabel"
        aria-hidden="true"
    >
        <div class="modal-dialog">
            <form
                method="POST"
                action="{{ route("tramites.cambiarPrioridad") }}"
            >
                @csrf
                <input
                    type="hidden"
...
@push("scripts")
    <script>
        ...
        function darDeBajaTramite(idTramite) {
            if (
                confirm('¿Estás seguro de que deseas dar de baja este trámite?')
            ) {
