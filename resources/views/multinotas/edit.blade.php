    ...
    <div class="container-xxl" style="margin-left: 20%; margin-right: 20%">
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="card">
                    <form
                        id="form-previsualizar-cambios"
                        method="POST"
                        action="{{ route("multinotas.previsualizarCambiosMultinota") }}"
                        novalidate
                    >
                        @csrf
                        <div
                                    ...
                                    >
                                        ...
                                        @else
                                            <input
                                                ...
                                                type="text"
                                                disabled
                                                value="{{ $multinotaSelected->codigo }}"
                                            />
                                        @endif
                                    </div>
                                    <div
                                    ...
                                    >
                                        ...
                                        @else
                                            <input
                                                ...
                                                id="nombre-tramite"
                                                type="text"
                                                value="{{ $multinotaSelected->nombre }}"
                                                required
                                            />
                                        @endif
                                    </div>
                                        ...
                                        >
                                            @if ($multinotaSelected->id_multinota_servicio == null)
                                                ...
                                                @foreach ($multinotaServicios as $ms)
                                                    <option
                                                        value="{{ $ms->id_multinota_servicio }}"
                                                    >
                                                        {{ $ms->nombre }}
                                                    </option>
                                                @endforeach
                                ...
                                >
                                    <div style="display: flex; gap: 15px">
                                        ...
                                        <input
                                            ...
                                            id="publico"
                                            type="checkbox"
                                            {{ $multinotaSelected->publico == 1 ? "checked" : "" }}
                                        />
                                    </div>
                                    <div style="display: flex; gap: 15px">
                                        <label style="font-weight: bold">
                                        ...
                                        <input
                                            name="llevaDocumentacion"
                                            id="lleva-documentacion"
                                            type="checkbox"
                                            {{ $multinotaSelected->lleva_documentacion == 1 ? "checked" : "" }}
                                        />
                                    </div>
                                    <div style="display: flex; gap: 15px">
                                        <label style="font-weight: bold">
                                        ...
                                        <input
                                            ...
                                            id="muestra-mensaje"
                                            type="checkbox"
                                            {{ $multinotaSelected->muestra_mensaje == 1 ? "checked" : "" }}
                                        />
                                    </div>
                                </div>
                            </div>
                        ...
                        >
                            <button
                                type="submit"
                                id="boton-previsualizar-cambios"
                                class="btn btn-secondary"
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                    ...
                    <div id="seccion-boton-volver-editar" class="hidden">
                        <div class="flex justify-between">
                            ...
                            <form
                                method="GET"
                                action="{{ route("multinotas.guardarMultinota", $multinotaSelected->id_tipo_tramite_multinota) }}"
                            >
                                @csrf
                                <button
                                    id="boton-guardar-multinota"
                                    class="btn btn-secondary"
                                >
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
...
@push("scripts")
    <script>
        ...
        const divBotonVolver = document.getElementById(
            'seccion-boton-volver-editar',
        );
        const divDetalle = document.getElementById('detalle-multinota');

        document
            .getElementById('boton-volver-editar')
        ...
        $(document).ready(function () {
            // Add section handler
            $('#secciones-container').on(
                'click',
                '#boton-agregar-seccion',
                function (event) {
                    event.preventDefault();
                    const idSeccionSeleccionada = $(
                        '#secciones-precargadas',
                    ).val();
                ...
                $(this)
                    ...
                    .each(function () {
                        ...
                        switch (nombreCampo) {
                            case 'categoria':
                                mensajeError =
                                    'Debe ingresar la categoría de la multinota';
                                break;
                            case 'subcategoria':
                                mensajeError =
                                    'Debe ingresar la subcategoría de la multinota';
                                break;
                            case 'nombre':
                                mensajeError =
                                    'Debe ingresar el nombre de trámite de la multinota';
                                break;
                            case 'codigo':
                                mensajeError =
                                    'Debe ingresar un código para la multinota';
                                break;
                            default:
                                break;
                        }
                        ...
                        for (const c of codigos) {
                            if ($('#codigo').val() == c && isEditar == false) {
                                mensajeError =
                                    'Debe ingresar un código distinto debido a que el ingresado ya está registrado en el sistema.';
        ...
        document
            .querySelector('.secciones-container')
